...
final class Block_Migration_Journal_Store {
	...
	public static function persist_transition(array $record,int $parent_event_id):array|\WP_Error{
		...
		if((string)($parent['record']['journal_id']??'')!==(string)($record['journal_id']??''))return new \WP_Error('bdc_kb_block_store_identity','Journal pai divergente.');
		$latest=self::latest_for_post((int)($record['post_id']??0));
		if($latest instanceof \WP_Error||(int)($latest['event_id']??0)!==$parent_event_id)return new \WP_Error('bdc_kb_block_store_chain','Cadeia não linear.');
	...
	public static function read_event(int $event_id):array|\WP_Error{
		...
		return array('event_id'=>(int)($m->meta_id??0),'parent_event_id'=>(int)($d['parent_event_id']??0),'actor_id'=>(int)($d['actor_id']??0),'created_at_gmt'=>(string)($d['created_at_gmt']??''),'record'=>$r);
	}
	...
	private static function append_event(array $record,int $parent):array|\WP_Error{
		...
		$rb=self::read_event($mid);if(!($rb instanceof \WP_Error)&&(string)($rb['record']['journal_hash']??'')===(string)($record['journal_hash']??''))return $rb;
